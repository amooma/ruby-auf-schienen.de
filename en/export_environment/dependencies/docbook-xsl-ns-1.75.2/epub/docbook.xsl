  ...
  <xsl:template name="opf.manifest">
    <xsl:element namespace="http://www.idpf.org/2007/opf" name="manifest">
     ...
     <xsl:if test="$epub.embedded.font != ''">
      ...
      <xsl:call-template name="opf.admonlist"/>
    </xsl:element>
  </xsl:template>

  <xsl:template name="opf.admonlist">
    <xsl:variable name="format">
      <xsl:call-template name="guess-media-type">
        <xsl:with-param name="ext" select="$admon.graphics.extension"/>
      </xsl:call-template>
    </xsl:variable>  
    <xsl:if test="//d:note">
		<xsl:call-template name="opf.reference.admonition">
			<xsl:with-param name="imgname">note</xsl:with-param>
			<xsl:with-param name="format" select="$format"/>
		</xsl:call-template>
    </xsl:if>
    <xsl:if test="//d:tip">
		<xsl:call-template name="opf.reference.admonition">
			<xsl:with-param name="imgname">tip</xsl:with-param>
			<xsl:with-param name="format" select="$format"/>
		</xsl:call-template>
    </xsl:if>
    <xsl:if test="//d:warning">
		<xsl:call-template name="opf.reference.admonition">
			<xsl:with-param name="imgname">warning</xsl:with-param>
			<xsl:with-param name="format" select="$format"/>
		</xsl:call-template>
    </xsl:if>
    <xsl:if test="//d:caution">
		<xsl:call-template name="opf.reference.admonition">
			<xsl:with-param name="imgname">caution</xsl:with-param>
			<xsl:with-param name="format" select="$format"/>
		</xsl:call-template>
    </xsl:if>
    <xsl:if test="//d:important">
		<xsl:call-template name="opf.reference.admonition">
			<xsl:with-param name="imgname">important</xsl:with-param>
			<xsl:with-param name="format" select="$format"/>
		</xsl:call-template>
    </xsl:if>
  </xsl:template>

  <xsl:template name="opf.reference.admonition">
    <xsl:param name="format"/>
	<xsl:param name="imgname"/>

    <xsl:variable name="filename" select="concat($admon.graphics.path, $imgname, $admon.graphics.extension)"/>

    <xsl:element namespace="http://www.idpf.org/2007/opf" name="item">
      <xsl:attribute name="id"> <xsl:value-of select="concat(generate-id(.), 'admon', $imgname)"/> </xsl:attribute>
